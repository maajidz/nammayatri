...
getPurchasedHistory (Id driverId) mbLimit mbOffset hasQuatityLeft = do
  findAllWithOptionsKV
    ...
    (if hasQuatityLeft then Se.Asc BeamDC.createdAt else Se.Desc BeamDC.createdAt)
    mbLimit
    mbOffset
...
updateQuantityLeft (Id purchaseId) quantityLeft = do
  now <- getCurrentTime
  updateWithKV
    [ Se.Set BeamDC.updatedAt now,
      Se.Set BeamDC.quantityLeft quantityLeft
    ]
    [Se.Is BeamDC.id $ Se.Eq purchaseId]
