...
mkCoinsDetails driverId' = do
  ...
  case latestActiveCoinPurchase of
    Just purchase -> do
      planLinkedToPurchase <- CQCP.getCoinPlanDetails (Id purchase.coinPlanId) >>= fromMaybeM (InternalError $ "No coin plan found with id : " <> purchase.coinPlanId)
      ...
      mapMaybe
        ( \invoice -> do
            ...
            case (invoice.bankErrorMessage, invoice.bankErrorCode, mapDfee M.!? invoice.driverFeeId, isExpiredError) of
              (Just message, Just code, Just dfee, False) -> do
                Just
                  ErrorEntity
                    { message = message,
                      code = code,
