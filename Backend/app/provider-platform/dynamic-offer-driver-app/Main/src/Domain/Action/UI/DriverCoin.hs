...
getCoinsByDriveId :: (MonadFlow m, CacheFlow m r) => Id SP.Person -> m Int
...
getCoinEventSummary (driverId, merchantId) dateInUTC = do
  ...
  let dateInIst = addUTCTime (-1 * secondsToNominalDiffTime transporterConfig.timeDiffFromUtc) dateInUTC
  ...
  let expiringDays_ = coinConfig.trackExpiry `div` 86400
  pure
    CoinTransactionRes
...
getCoinUsageSummary (driverId, merchantId) = do
  ...
  where
    ...
    toUsageHistoryItem historyItem = do
      coinPlan <- CoinPlan.getCoinPlanDetails (Id historyItem.coinPlanId) >>= fromMaybeM (InternalError $ "plan details not found" <> historyItem.coinPlanId)
      pure
...
useCoinsHandler (driverId, merchantId) PurchasePlanRequest {..} = do
  ...
  coinPlan <- CoinPlan.getCoinPlanDetails (Id coinPlanId) >>= fromMaybeM (InternalError $ "plan details not found" <> coinPlanId)
