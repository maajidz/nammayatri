...
verifyDL isDashboard mbMerchant (personId, _, merchantOpCityId) req@DriverDLReq {..} = do
  ...
  when
    ...
    $ do
      ...
      resp <-
        Verification.extractDLImage merchantOpCityId $
...
verifyDLFlow person merchantOpCityId onboardingDocumentConfig dlNumber driverDateOfBirth imageId1 imageId2 dateOfIssue = do
  ...
  verifyRes <-
    Verification.verifyDLAsync merchantOpCityId $
    ...
    linkDl person = do
      if verificationReq.imageExtractionValidation == Domain.Skipped
        ...
        then do
          ...
          now <- getCurrentTime
          id <- generateGUID
          onboardingDocumentConfig <- QODC.findByMerchantOpCityIdAndDocumentType person.merchantId DTO.DL >>= fromMaybeM (OnboardingDocumentConfigNotFound person.merchantId.getId (show DTO.DL))
