...
generateAadhaarOtp isDashboard mbMerchant personId merchantOpCityId req = do
  ...
  transporterConfig <- CTC.findByMerchantOpCityId merchantOpCityId >>= fromMaybeM (TransporterConfigNotFound person.merchantId.getId)
  unless (isDashboard || tried < transporterConfig.onboardingTryLimit) $ throwError (GenerateAadhaarOtpExceedLimit personId.getId)
  res <- AadhaarVerification.generateAadhaarOtp merchantOpCityId req
...
verifyAadhaarOtp mbMerchant personId merchantOpCityId req = do
  ...
  case mtIdAndAadhaarHash of
    Just (tId, aadhaarNumberHash) -> do
      ...
      res <- AadhaarVerification.verifyAadhaarOtp merchantOpCityId aadhaarVerifyReq
...
fetchAndCacheAadhaarImage :: (MonadFlow m, MonadTime m, MonadReader r m, HasField "s3Env" r (S3.S3Env m), CacheFlow m r, EsqDBFlow m r) => Person.Person -> Id DMOC.MerchantOperatingCity -> DriverInformation -> m (Maybe Text)
fetchAndCacheAadhaarImage driver merchantOpCityId driverInfo =
    ...
    then case driverInfo.compAadhaarImagePath of
      ...
      Nothing -> do
        ...
        case aadhaarVerification.driverImagePath of
          Nothing -> backfillAadhaarImage driver merchantOpCityId aadhaarVerification
          Just imgPath -> do
            uploadedImage <- CQDI.getDriverImageByDriverId driverInfo.driverId imgPath
            let imageType = getImageExtension uploadedImage
            (compImage, resultComp) <- uploadCompressedAadhaarImage driver merchantOpCityId uploadedImage imageType
            case resultComp of
...
uploadCompressedAadhaarImage person merchantOpCityId image imageType = do
  transporterConfig <- CTC.findByMerchantOpCityId merchantOpCityId >>= fromMaybeM (TransporterConfigNotFound (getId (person.merchantId)))
