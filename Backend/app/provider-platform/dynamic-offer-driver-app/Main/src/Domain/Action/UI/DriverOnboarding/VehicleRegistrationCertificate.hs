...
verifyRC isDashboard mbMerchant (personId, merchantId, merchantOpCityId) req@DriverRCReq {..} mbVariant = do
  ...
  onboardingDocumentConfig <- SCO.findByMerchantOpCityIdAndDocumentType merchantOpCityId ODC.RC >>= fromMaybeM (OnboardingDocumentConfigNotFound person.merchantId.getId (show ODC.RC))
  ...
  transporterConfig <- QTC.findByMerchantOpCityId merchantOpCityId >>= fromMaybeM (TransporterConfigNotFound person.merchantId.getId)
  ...
  when
    ...
    $ do
      ...
      resp <-
        Verification.extractRCImage merchantOpCityId $
...
verifyRCFlow person merchantOpCityId imageExtraction rcNumber imageId dateOfRegistration multipleRC mbVariant = do
  ...
  verifyRes <-
    Verification.verifyRCAsync merchantOpCityId $
...
onVerifyRC verificationReq output = do
  ...
  if verificationReq.imageExtractionValidation == Domain.Skipped
    ...
    then IVQuery.updateExtractValidationStatus verificationReq.requestId Domain.Failed >> return Ack
      ...
      now <- getCurrentTime
      id <- generateGUID
      rCConfigs <- SCO.findByMerchantOpCityIdAndDocumentType person.merchantId ODC.RC >>= fromMaybeM (OnboardingDocumentConfigNotFound person.merchantId.getId (show ODC.RC))
      rCInsuranceConfigs <- SCO.findByMerchantOpCityIdAndDocumentType person.merchantId ODC.RCInsurance >>= fromMaybeM (OnboardingDocumentConfigNotFound person.merchantId.getId (show ODC.RCInsurance))
...
linkRCStatus (driverId, merchantId, merchantOpCityId) req@RCStatusReq {..} = do
  ...
  transporterConfig <- QTC.findByMerchantOpCityId merchantOpCityId >>= fromMaybeM (TransporterConfigNotFound merchantId.getId)
