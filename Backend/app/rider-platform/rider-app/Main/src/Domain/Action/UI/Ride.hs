...
editLocation rideId (_, merchantId) req = do
  ...
  whenJust req.origin $ \pickup -> do
    ...
    pickupMapForBooking <- SLM.buildPickUpLocationMapping startLocation.id bookingId.getId DLM.BOOKING -- create loc map table
    QLM.create pickupMapForBooking
    pickupMapForRide <- SLM.buildPickUpLocationMapping startLocation.id ride.id.getId DLM.RIDE -- create loc map table
