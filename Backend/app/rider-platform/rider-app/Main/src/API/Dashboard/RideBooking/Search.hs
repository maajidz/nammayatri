...
callSearch merchantId personId req = do
  ...
  merchantOperatingCity <- withFlowHandlerAPI $ SMOC.findByMerchantIdAndCity m.id m.city >>= fromMaybeM (MerchantOperatingCityNotFound ("merchId: " <> m.id.getId <> " ,city: " <> Kernel.Utils.Common.show m.city))
