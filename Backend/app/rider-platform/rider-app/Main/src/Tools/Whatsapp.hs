...
whatsAppOptAPI :: ServiceFlow m r => Id DMOC.MerchantOperatingCity -> GupShup.OptApiReq -> m APISuccess
  ...
  where
    ...
    getProviderConfig provider = do
      merchantWhatsappServiceConfig <-
        QMSC.findByMerchantOperatingCityIdAndService merchantOperatingCityId (DMSC.WhatsappService provider)
...
whatsAppOtpApi :: ServiceFlow m r => Id DMOC.MerchantOperatingCity -> GupShup.SendOtpApiReq -> m GupShup.SendOtpApiResp
whatsAppOtpApi merchantOperatingCityId = GupShup.whatsAppOtpApi handler
  where
    handler = GupShup.WhatsappHandler {..}
