...
runWithServiceConfig func getCfg merchantOperatingCityId req = do
  ...
  merchantNotificationServiceConfig <-
    QMSC.findByMerchantOperatingCityIdAndService merchantOperatingCityId (DMSC.NotificationService $ getCfg merchantConfig)
...
notifyOnDriverOfferIncoming estimateId quotes person = do
  m <- CQM.findById person.merchantId >>= fromMaybeM (MerchantNotFound person.id.getId)
...
notifyOnNewMessage booking message = do
  ...
  notifyPersonWithMutableContent merchantOperatingCity.id notificationData
