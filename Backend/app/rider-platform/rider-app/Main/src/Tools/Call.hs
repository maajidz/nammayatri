...
runWithServiceConfig func getCfg merchatOperatingCityId req = do
  merchantConfig <- QMSUC.findByMerchantOperatingCityId merchatOperatingCityId >>= fromMaybeM (MerchantServiceUsageConfigNotFound merchatOperatingCityId.getId)
  merchantCallServiceConfig <-
    QMSC.findByMerchantOperatingCityIdAndService merchatOperatingCityId (DMSC.CallService $ getCfg merchantConfig)
