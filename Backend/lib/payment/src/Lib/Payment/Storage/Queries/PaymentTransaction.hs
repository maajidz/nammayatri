...
findNewTransactionByOrderId (Id orderId) = findAllWithOptionsKV [Se.And [Se.Is BeamPT.txnUUID $ Se.Eq Nothing, Se.Is BeamPT.orderId $ Se.Eq orderId]] (Se.Desc BeamPT.createdAt) (Just 1) Nothing <&> listToMaybe
