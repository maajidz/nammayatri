  ...
  linearLayout
    ...
    , margin $ Margin 116 10 10 0
        ...
        , orientation VERTICAL
          ...
          , commonTV push (getString ENJOY_THESE_BENEFITS) Color.black800 (FontStyle.subHeading2 TypoGraphy) 0 LEFT true
          , linearLayout
            [ width WRAP_CONTENT
            , height WRAP_CONTENT
            , orientation VERTICAL
              ...
              ([(getString ZERO_COMMISION), (getString EARN_TODAY_PAY_TOMORROW)] 
                  <> if state.data.config.enableIntroductoryView
  ...
  , padding $ Padding 20 20 20 if length state.data.joinPlanData.allPlans == 1 then 16 else 0
      ...
      , orientation VERTICAL
        ...
        , linearLayout
          ...
          , height WRAP_CONTENT
          , gravity CENTER_VERTICAL
          , visibility if state.data.config.myPlanYoutubeLink /= "" then VISIBLE else GONE
          , onClick (\action -> do
                        _ <- push action
                        ...
                        _ <- JB.openUrlInApp $ case getValueToLocalNativeStore LANGUAGE_KEY of
                                          "EN_US" -> state.data.config.myPlanYoutubeLink
                                          "KN_IN" -> state.data.config.myPlanYoutubeLink
                                          _ -> state.data.config.myPlanYoutubeLink
                        pure unit
                        ) (const NoAction)
          ][ textView $
              [ height WRAP_CONTENT
...
myPlanBodyview push state =
  ...
  , width MATCH_PARENT
    ...
    , planCardView push state.data.myPlanData.planEntity state.data.myPlanData.planEntity.isSelected (not isFreezed) TogglePlanDescription state.props.isSelectedLangTamil false true true Nothing false
...
lottieJsonAccordingToLang isOnFreeTrial = do
