...
import Components.LocationListItem.Controller (dummyLocationListState, dummyAddress)
...
homeScreenFlow = do
  ...
  case flow of
    ...
    LOCATION_SELECTED item addToRecents-> do
        ...
        _ <- pure $ spy "destination prediction clicked item ---->>> " item
        ...
        if (addToRecents) then do
          ...
          _ <- fetchAndModifyLocationLists bothLocationChangedState.data.savedLocations
          pure unit
          else pure unit
        (GlobalState globalState) <- getState
    ...
    SEARCH_LOCATION input state -> do
      ...
      let sortedtWithDistance =  recentDistance state.data.recentSearchs.predictionArray state.props.sourceLat state.props.sourceLong
      let recentLists = if state.props.isSource == Just true then sortedtWithDistance else state.data.destinationSuggestions
      ...
      _ <- pure $ spy "filteredPredictionList zxc" filteredPredictionList 
      _ <- pure $ spy "filteredRecentsList zxc" filteredRecentsList 
    ...
    FCM_NOTIFICATION notification state-> do
        ...
        case notification of
            "TRIP_STARTED"        -> do -- OTP ENTERED
                                      ...
                                      let currentSourceGeohash = encodeGeohash srcLat srcLon state.data.config.suggestedTripsAndLocationConfig.geohashPrecision
    ...
    UPDATE_LOCATION_NAME state lat lon -> do
      ...
      if (fromMaybe "" sourceServiceabilityResp.geoJson) /= "" && (fromMaybe "" sourceServiceabilityResp.geoJson) /= state.data.polygonCoordinates && pickUpPoints /= state.data.nearByPickUpPoints then do
        ...
        let currentLocationItem = Just (dummyLocationListState{
                            ...
                            , title = (fromMaybe "" ((split (Pattern ",") (placeDetails.formattedAddress)) !! 0))
                            , subTitle = (drop ((fromMaybe 0 (indexOf (Pattern ",") (placeDetails.formattedAddress))) + 2) (placeDetails.formattedAddress))
                            , placeId = placeDetails.placeId
                            , lat = Just (if state.props.currentStage /= ConfirmingLocation then lat 
                                        else if (state.props.isSource == Just true) then state.props.sourceLat
                                        else state.props.destinationLat)
                            , lon = Just (if state.props.currentStage /= ConfirmingLocation then lon 
                                         else if (state.props.isSource == Just true) then state.props.sourceLong
    ...
    UPDATE_SAVED_LOCATION -> do
      ...
      case savedLocationResp of
        Right (SavedLocationsListRes listResp) -> do
          _ <- fetchAndModifyLocationLists (AddNewAddress.getSavedLocations listResp.list)
          homeScreenFlow
...
savedLocationFlow = do
  ...
  case flow of
    ADD_NEW_LOCATION state-> do
      ...
      let currentGeoHash = encodeGeohash (fromMaybe 0.0 $ fromString $ getValueToLocalNativeStore LAST_KNOWN_LAT) (fromMaybe 0.0 $ fromString $ getValueToLocalNativeStore LAST_KNOWN_LON) newState.homeScreen.data.config.suggestedTripsAndLocationConfig.geohashPrecision
  ...
  case item.locationItemType of
    Just PREDICTION -> do
        ...
        when (state.props.isSource == Just false) $ do
          setSuggestionsMapInLocal item lat lon serviceable state
    _ -> do 
        saveToRecents item (fromMaybe 0.0 item.lat) (fromMaybe 0.0 item.lon) serviceable
        when (state.props.isSource == Just false) $ do
          setSuggestionsMapInLocal item (fromMaybe 0.0 item.lat) (fromMaybe 0.0 item.lon) serviceable state
  pure unit
...
saveToRecents item lat lon serviceability = do
  ...
  if serviceability then do
    modifyScreenState $ HomeScreenStateType (\homeScreen -> homeScreen{ data { recentSearchs { predictionArray = addToRecentSearches item{lat = Just lat, lon = Just lon} recentPredictionsObject.predictionArray}}
                                                                      })
...
setSuggestionsMapInLocal item lat lon serviceability state= do
    ...
    when (serviceability) $ do
      let currentSourceGeohash = encodeGeohash currentState.homeScreen.props.sourceLat currentState.homeScreen.props.sourceLong state.data.config.suggestedTripsAndLocationConfig.geohashPrecision
      let destinationWithLatLong = item{lat = Just lat, lon = Just lon}
      let currentMap = currentState.homeScreen.data.suggestionsData.suggestionsMap
...
getSuggestionsMapFromLocal :: LazyCheck -> SuggestedDestinations 
getSuggestionsMapFromLocal lazycheck = case (fetchSuggestionsFromLocal "SUGGESTIONS_MAP") of 
                    Right a -> a
                    Left err -> Map.empty
...
fetchAndModifyLocationLists savedLocationResp = do 
    ...
    let suggestionsConfig = state.data.config.suggestedTripsAndLocationConfig
        savedLocationWithHomeOrWorkTag = (filter (\listItem ->  (listItem.prefixImageUrl == "ny_ic_home_blue,https://assets.juspay.in/nammayatri/images/user/ny_ic_home_blue.png"|| (listItem.prefixImageUrl == "ny_ic_work_blue,https://assets.juspay.in/nammayatri/images/user/ny_ic_work_blue.png"))) (savedLocationResp))
        ...
        currentGeoHash = if (state.props.sourceLat /= 0.0 && state.props.sourceLong /= 0.0) 
                          then encodeGeohash state.props.sourceLat state.props.sourceLong suggestionsConfig.geohashPrecision
                          else encodeGeohash (fromMaybe 0.0 $ fromString $ getValueToLocalNativeStore LAST_KNOWN_LAT) (fromMaybe 0.0 $ fromString $ getValueToLocalNativeStore LAST_KNOWN_LON) suggestionsConfig.geohashPrecision
        ...
        sugestedFinalList =  suggestedDestinationsArr <> (Arr.take (30 - (length suggestedDestinationsArr)) recentSearchesWithoutSuggested)
    ...
    modifyScreenState $ HomeScreenStateType (\homeScreen -> homeScreen{ data
                                                                        ...
                                                                        , tripSuggestions = currentGeoHashDestinations.tripSuggestions
                                                                        }
                                                                      })
    pure unit
