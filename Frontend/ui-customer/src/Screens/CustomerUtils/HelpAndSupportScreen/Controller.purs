...
import Helpers.Utils (validateEmail,strLenWithSpecificCharacters,getTime,toString, getCommonAssetStoreLink, getAssetStoreLink)
...
topicsList state =  (if state.data.config.showIssueOption 
  then 
    ...
    , categoryImageUrl : "ny_ic_clip_board," <> (getCommonAssetStoreLink FunctionCall) <> "ny_ic_clip_board.png"
    , categoryId : "5"
    },
    { categoryLabel : "CALL_SUPPORT"
    , categoryName : (getString CONTACT_SUPPORT)
    , categoryImageUrl : "ny_ic_help," <> (getAssetStoreLink FunctionCall) <> "ny_ic_help.png"
    , categoryId : "6"
    }]
...
reportsList state = [
  ...
  , categoryName : (getString REPORTED <> " : " <> (toString (DA.length (state.data.ongoingIssueList))))
  , categoryImageUrl : "ny_ic_reported," <> (getCommonAssetStoreLink FunctionCall) <> "ny_ic_reported.png"
  , categoryId : "1"
  },
  { categoryLabel : "RESOLVED"
  , categoryName : (getString RESOLVED <> " : " <> (toString (DA.length (state.data.resolvedIssueList))))
  , categoryImageUrl : "ny_ic_resolved," <> (getCommonAssetStoreLink FunctionCall) <> "ny_ic_resolved.png"
  , categoryId : "2"
  } 
]