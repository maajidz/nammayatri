...
import Helpers.Utils (toString, getAssetStoreLink, getCommonAssetStoreLink, strLenWithSpecificCharacters)
import JBridge as JB
import Language.Strings (getString)
import Language.Types (STR(..))
import Prelude (when, Unit, bind, const, discard, map, pure, unit, show, ($), (-), (/=), (<<<), (<=), (<>), (==), (||), (<), (<>))
        ...
        headerConfig {
          headTextConfig {
            text = if (state.data.issueListType == ST.REPORTED_ISSUES_MODAL) 
              then (getString REPORTED_ISSUES) <> (" : ") <> (toString (DA.length state.data.ongoingIssueList)) 
              else (getString RESOLVED_ISSUES) <> (" : ") <> (toString (DA.length state.data.resolvedIssueList))
          }
        },
        issueViewConfig {
          thirdTextConfig {
