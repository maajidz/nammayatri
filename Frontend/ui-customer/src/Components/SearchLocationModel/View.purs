...
import PrestoDOM.Properties (cornerRadii, sheetState, focusable, focusableInTouchMode)
...
chooseYourRideView :: forall w. (Action -> Effect Unit) -> SearchLocationModelState -> PrestoDOM (Effect Unit) w
  ...
  linearLayout
    ...
    [ linearLayout
      ...
      , orientation VERTICAL
          ...
          ) (chooseYourRideConfig state).quoteList
      )
    ]
  ...
  , clickable isClickable
      ...
      , orientation HORIZONTAL
        ...
        , linearLayout
          ...
          , padding (PaddingHorizontal 5 5)
          , visibility if (state.rentalStage == RentalSlab) then VISIBLE else GONE
          , onClick (\action -> timePicker push $ TimePicker ) (const NoAction)
          ][  imageView
              [ height $ V 16
              , width $ V 16
